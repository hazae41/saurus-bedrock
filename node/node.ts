...
const privateKeyFormat: { format: "der", type: "sec1" } = { format: "der", type: "sec1" }
const publicKeyFormat: { format: "der", type: "spki" } = { format: "der", type: "spki" }

const loadPublicKey = (key: Buffer) => createPublicKey({ key, ...publicKeyFormat })
const loadPrivateKey = (key: Buffer) => createPrivateKey({ key, ...privateKeyFormat })

const genECKeyPair = () => generateKeyPairSync("ec", { namedCurve: "secp384r1" })
...
function handle(method: number, buffer: Buffer) {
    if (method === 0) return deflateSync(buffer)
    if (method === 1) return inflateSync(buffer)
